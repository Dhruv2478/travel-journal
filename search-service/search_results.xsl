...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <xsl:choose>
            <xsl:when test="count(SearchResults/destination) > 0">
                <xsl:for-each select="SearchResults/destination">
                    <div class="post">
                        <div class="post-meta">
                            <span class="author">
            ...
            <xsl:otherwise>
                <p class="no-posts">No destinations found matching your search.</p>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
</xsl:stylesheet>
